...
class AmisReceiptRiskService
{
    ...
    public function assess(array $context): array
    {
        ...
        $now = $context['now'];
        if ($transactionAt->year > $now->year) {
            $add('INVALID_DATE', 'block', "The transaction year cannot be later than {$now->year}.");
        } elseif ($transactionAt->gt($now)) {
            $add('FUTURE_DATE_CURRENT_YEAR', 'review', 'The transaction date is later in the current year. Finance must confirm it against the receipt.');
